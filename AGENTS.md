...
Do not run FFT/Sobel resolution detection on extracted video frames. Estimate the apparent source-pixel step from representative quantized frames using gradient-peak spacing, infer the source-art grid, double that grid, and recommend the nearest 32×32, 64×64, 128×128, or 256×256 preset. Keep all presets manually selectable. A selected preset is always its true logical resolution: 256×256 means 256×256 independently reconstructed pixels. Use a rigid full-frame grid, robust median/majority cell sampling, one shared palette, deterministic palette mapping, and conservative temporal shade stabilization. Keep the full video framing so processing does not introduce scale or position drift; never independently shift animation grid lines.

After recovering a still image's native pixel lattice, crop transparent margins with a small native-pixel safety margin before fitting a standard output canvas. Never upscale a recovered still sprite merely to fill the target. Animation fixed-grid processing keeps the full source frame and does not crop or recenter individual frames.
